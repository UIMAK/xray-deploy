...
# ---------------------------------------------------------------------------
# 启用 / 禁用 logrotate —— 三条路径(手动开关 / loglevel=none 自动禁用 / 切出 none 自动恢复)
# 统一走这两个函数, 不再各自拼 _state_set + rm + cat。
#
# **返回码三态, 因为"哪一步失败"决定调用方该做什么:**
#   0 = 文件动作成功 + state 记账成功
#   1 = **文件动作失败** —— 真实世界未改变(禁用时轮换仍在跑 / 启用时轮换未生效)
#   2 = **文件动作成功, 仅 state 记账失败** —— 真实世界已改变, 只是记录没写上
# 旧式 `if ! _logrotate_disable` 调用点不受影响(任何非零仍算失败), 需要区分因果的才捕获 rc。
#
# **顺序规则: 先做真实世界的动作, 再记账**(与 _geo_set_auto_update 一致)。两种失败残局代价
# 不对称: 若先记账, 写文件失败会留下"state=on 但没有配置"(用户以为在轮换, 磁盘被撑满)或
# "state=off 但文件仍在"(logrotate 继续轮换而 UI 说已停, PR #27 P1 的分裂); 反过来失败只是
# 显示滞后且必定告警。
#
# 与 _geo_set_auto_update 的差异: 那里 state 失败要回滚 crontab 行(遗留 entry 会无人值守执行
# 本脚本); 这里遗留的只是一份 logrotate 配置, 行为恰是用户刚要求的、安装时默认也会写, 故
# 不回滚只告警。**不要改成让调用方 `[ -e "$LOGROTATE_CONF" ]` 反推原因** —— 那是"同一条件
# 在各调用点各自解释"的反模式。
# ---------------------------------------------------------------------------
_logrotate_enable() {
